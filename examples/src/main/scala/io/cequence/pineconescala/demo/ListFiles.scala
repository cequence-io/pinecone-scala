...
object ListFiles extends PineconeDemoApp {
  ...
  override protected def exec = {
    ...
    for {
      ...
      _ <- pineconeAssistantFileService.uploadFile(assistantName, file)
      files <- pineconeAssistantFileService.listFiles(assistantName)
    } yield files.foreach(println)
  }
