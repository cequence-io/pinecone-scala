      ...
      try {
        json.validate[T] match {
          ...
          case JsError(errors) =>
            ...
            throw new PineconeScalaClientException(s"Unexpected JSON:\n'${Json.prettyPrint(json)}'. Cannot be parsed due to: $errorString")
        }
      } catch {
        case e: Exception => throw new PineconeScalaClientException(s"Error thrown while processing a JSON '$json'. Cause: ${e.getMessage}")
      }
