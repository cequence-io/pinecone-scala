...
import io.cequence.pineconescala.JsonUtil.toJson
import io.cequence.pineconescala.{PineconeScalaClientException, PineconeScalaClientTimeoutException, PineconeScalaClientUnknownHostException}
...
trait WSRequestHelper extends WSHelper {
    ...
    execRequestRaw(
      ...
      case Left(response) =>
        try {
          ...
          case _: JsonParseException => throw new PineconeScalaClientException(s"$serviceName.${endPointForLogging.map(_.toString).getOrElse("")}: '${response.body}' is not a JSON.")
          case _: JsonMappingException => throw new PineconeScalaClientException(s"$serviceName.${endPointForLogging.map(_.toString).getOrElse("")}: '${response.body}' is an unmappable JSON.")
        }
  ...
  private def execRequestRaw(
    ...
    case e: TimeoutException => throw new PineconeScalaClientTimeoutException(s"$serviceName.${endPointForLogging.map(_.toString).getOrElse("")} timed out: ${e.getMessage}.")
    case e: UnknownHostException => throw new PineconeScalaClientUnknownHostException(s"$serviceName.${endPointForLogging.map(_.toString).getOrElse("")} cannot resolve a host name: ${e.getMessage}.")
  }
  ...
  protected def handleErrorResponse[T](response: RichResponse[T]) =
    response match {
      ...
      case Right((errorCode, message)) => throw new PineconeScalaClientException(s"Code ${errorCode} : ${message}")
    }
  ...
  protected def handleNotFoundAndError[T](response: Either[T, (Int, String)]): Option[T] =
    response match {
      ...
      case Right((errorCode, message)) =>
        if (errorCode == 404) None else throw new PineconeScalaClientException(s"Code ${errorCode} : ${message}")
    }
